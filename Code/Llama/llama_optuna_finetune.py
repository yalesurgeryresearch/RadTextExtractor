...
# Load the universal training argumentsß
...
def objective(trial):
    ...
    learning_rate = trial.suggest_categorical(
        "learning_rate", search_space_dict["learning_rate"]
    )
    rank = trial.suggest_categorical("rank", search_space_dict["rank"])
    alpha = trial.suggest_categorical("alpha", search_space_dict["alpha"])
    seed_shuffle = (
        trial.suggest_categorical("seed", search_space_dict["seed"])
        if "seed" in search_space_dict
        else None
    )
    num_samples = (
        trial.suggest_categorical("num_samples", search_space_dict["num_samples"])
        if "num_samples" in search_space_dict
        else None
    )
    ...
    validation_dataset = finetuning_dataset_from_jsonl(
        tokenizer, validation_path, inc, messages
    )
    ...
    collator = DataCollatorForCompletionOnlyLM(
        response_template_with_context, tokenizer=tokenizer
    )
