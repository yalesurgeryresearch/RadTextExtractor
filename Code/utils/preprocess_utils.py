...
SENT_KEYWORDS = ["aorta","aortic","ascending","descending","sov","root","aneurysm"," arch","valsalva","sinotubular"]
LABEL_DICT = {
    'Mid Ascending': 'ASC',
    'Annulus': 'ANN',
...
def check_numbers_sentence(sentence, nlp):
    ...
    for token in doc:
        # if re.match(r'-?\b\d+\b|-?\b\d+\.\d+\b', token.text) and not is_date_or_time_spacy(token):
        if re.match(r'\d', token.text) and not is_date_or_time_spacy(token) and not '/' in token.text:
...
def check_sample(sample, nlp):
    ...
    return sample["start_char"] > 0 and "node" not in sentence and check_sentence(sentence,nlp)

# Function to replace Date and Time entities with a placeholder
# def replace_date_time(sentence):
...
#     for token in tokens:
#         if  re.match(r'\d', token) and any(c in token for c in ['/',':','-']):
#             # print(f" token: {token} is a date or time")
#             date_times.append(token)
#             sentence = sentence.replace(token, '[DATE_TIME]')
#     return sentence, date_times

# def restore_date_time(sentence, date_times):
#     for date_time in date_times:
#         sentence = sentence.replace('[DATE_TIME]', date_time, 1)
#     return sentence
...
# # Function to restore the original numbers in the sentence
...
def extract_entities(sentence):
    ...
    for match in tag_pattern.finditer(sentence):
        ...
        if entity:
            entities.append({'start': start-5, 'end': end-4, 'text': text, 'labels': [entity]})
...
# def split_text_nltk(data_dic, sent_tokenizer,nlp):
#     narrative= data_dic['Narrative']
#     id = data_dic['AccessionNumber']
...
#             "end_char": sent_end,
#         }
#         sent_dic["needs_inference"] = check_sample(sent_dic, nlp)
...
def split_text_nltk_v2(data_dic, sent_tokenizer,nlp,id_variable):
    ...
    for (sent_start, sent_end), sentence in zip(spans, sentences):
        ...
        if "label" in data_dic:
            ...
            sent_dic["dimensions"] = sorted(sent_dic["dimensions"], key=lambda x: x["start"])
            if len(sent_dic["dimensions"]) > 0:
                ...
                for dim in reversed(sent_dic["dimensions"]):
                     sent_dic["labeled_sentence"] = sent_dic["labeled_sentence"][:dim["start"]] + f"<{LABEL_DICT[dim['labels'][0]]}>" + sent_dic["labeled_sentence"][dim["start"]:dim["end"]] + f"</{LABEL_DICT[dim['labels'][0]]}>" + sent_dic["labeled_sentence"][dim["end"]:]
