...
def compute_metrics_v2(p):
    custom_labels = {0: "O", 1: "ANN", 2: "SOV", 3: "STJ", 4: "ASC", 5: "PTB", 6: "ARC", 7: "PDC", 8: "DSC"}
    ...
    for prediction, label in zip(predictions, labels):
        ...
        true_labels.extend([int(l) for l in label if l != -100])  # Convert labels to int
        true_predictions.extend([int(p) for p, l in zip(prediction, label) if l != -100])  # Convert predictions to int
    
    # Get precision, recall, f1 score per label
    precision, recall, f1, _ = precision_recall_fscore_support(true_labels, true_predictions, labels=np.unique(true_labels), zero_division=0)
    
    # Map results to their corresponding labels
    unique_labels = [custom_labels[int(label)] for label in np.unique(true_labels)]  # Convert labels to Python int
    label_metrics = {
    ...
    macro_precision = float(np.mean([label_metrics[label]["precision"] for label in valid_labels]))
    macro_recall = float(np.mean([label_metrics[label]["recall"] for label in valid_labels]))
...
def calculate_metrics(narratives_data_list):
    ...
    for label in set(overall_tp.keys()).union(overall_fp.keys()).union(overall_fn.keys()):
        precision[label] = overall_tp[label] / (overall_tp[label] + overall_fp[label]) if (overall_tp[label] + overall_fp[label]) > 0 else 0
        recall[label] = overall_tp[label] / (overall_tp[label] + overall_fn[label]) if (overall_tp[label] + overall_fn[label]) > 0 else 0
