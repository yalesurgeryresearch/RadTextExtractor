...
ENTITY_TO_LABEL_DICT = {"O": 0, "ANN": 1, "SOV": 2, "STJ": 3, "ASC": 4, "PTB": 5, "ARC": 6, "PDC": 7, "DSC": 8}
...
def load_model(model_path_or_name):
    ...
    model = AutoModelForTokenClassification.from_pretrained(model_path_or_name, config=config)
    for param in model.parameters(): param.data = param.data.contiguous()
    return model


# Function to tokenize and format the data for BERT finetuning
def tokenize_and_format_data(file_path, tokenizer, dim = "dimensions", sent = "sentence", inc = "inclusion", max_length=384, labeled=False):
    ...
    def create_and_adjust_labels(entities, offset_mapping, attention_mask):
        ...
        for idx, (offset_start, offset_end) in enumerate(offset_mapping):
            ...
            for start, end, entity_type in entities:
                if offset_start == start or (offset_start > start and offset_end <= end):
                    label = entity_type
                    break
    ...
    for sample in json_data:
        if sample[inc]:
            ...
            if labeled:
                ...
                labels = create_and_adjust_labels(entities, encoding['offset_mapping'], encoding['attention_mask'])
...
def get_pred_sent(ner_results, sentence_text):
    ...
    for result in sorted_ner_results:
        if merged_results and merged_results[-1]['entity'] == result['entity'] and merged_results[-1]['end'] >= result['start']:
    ...
    for result in reversed(merged_results):
        sentence_text = sentence_text[:result['start']] + f"<{result['entity']}>{sentence_text[result['start']:result['end']]}</{result['entity']}>" + sentence_text[result['end']:]
