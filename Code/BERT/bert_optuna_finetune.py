...
def objective(trial):
    ...
    if num_tokenization:
        dim= f"dimensions_num_tokens"
        sent=f"sentence_num_tokens"
    
    # Prepare train and validation dataset
    train_dataset = tokenize_and_format_data(train_path, tokenizer, dim=dim, sent=sent, inc=inc, labeled=True)
    validation_dataset = tokenize_and_format_data(validation_path, tokenizer, dim=dim, sent=sent, inc=inc, labeled=True)
    
    # Set trial hyperparameters
    learning_rate = trial.suggest_categorical("learning-Rate", search_space_dict["learning-Rate"])
    batch_size = trial.suggest_categorical("batch-size", search_space_dict["batch-size"])
    dict_update = {
